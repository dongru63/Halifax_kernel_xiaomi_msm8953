...
KERNEL_DIR="HalifaxKernel"
KERNEL_NAME="HalifaxKernel"
KERNEL_VER="4.9"
OUT_DIR="outputdTiffany-4.9-Custom"
...
mkdir ~/$KERNEL_DIR/${KERNEL_NAME}_V${KV}-${DEVICE_CODENAME}-${KERNEL_VER}-Custom
echo " ";
echo " ";
echo " ";
echo " ";
...
cp -r template/. ~/$KERNEL_DIR/${KERNEL_NAME}_V${KV}-${DEVICE_CODENAME}-${KERNEL_VER}-Custom

cp $OUT_DIR/$IMAGES_PATH/Image.gz ~/$KERNEL_DIR/${KERNEL_NAME}_V${KV}-${DEVICE_CODENAME}-${KERNEL_VER}-Custom/kernel/Image.gz
cp $OUT_DIR/$DTB_PATH/msm8953-qrd-sku3-tiffany-nontreble.dtb ~/$KERNEL_DIR/${KERNEL_NAME}_V${KV}-${DEVICE_CODENAME}-${KERNEL_VER}-Custom/dtb-nontreble
cp $OUT_DIR/$DTB_PATH/msm8953-qrd-sku3-tiffany-treble.dtb ~/$KERNEL_DIR/${KERNEL_NAME}_V${KV}-${DEVICE_CODENAME}-${KERNEL_VER}-Custom/dtb-treble
...
##########################################
cd ~/$KERNEL_DIR/${KERNEL_NAME}_V${KV}-${DEVICE_CODENAME}-${KERNEL_VER}-Custom
zip -r9 ${KERNEL_NAME}_V${KV}-${DEVICE_CODENAME}-${KERNEL_VER}-Custom.zip * -x ${KERNEL_NAME}_V${KV}-${DEVICE_CODENAME}-${KERNEL_VER}-Custom.zip
